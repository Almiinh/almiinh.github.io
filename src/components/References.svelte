<script>
    import { onMount } from "svelte";
    const references = [
        {
            href: "index/CV FR Minh-Hoang HUYNH.pdf",
            src: "index/CV128.red.png",
            alt: "PDF",
        },
        {
            href: "https://www.linkedin.com/in/minh-hoang-alexis-huynh-621b52222/",
            src: "index/LINKEDIN.svg",
            alt: "LinkedIn",
        },
        {
            href: "https://github.com/Almiinh",
            src: "index/GITHUB.svg",
            dark: "index/GITHUB.white.svg",
            alt: "GitHub",
        },
    ];

    let darkMode = false;
    onMount(() => {
        darkMode = (window.matchMedia("(prefers-color-scheme: dark)").matches || localStorage.theme === "dark");
    });
</script>

<section class="references">
    {#each references as item}
        <a href={item.href}>
            {#if darkMode && }
                <img
                    src={item.src.split(".")[0] + ".white." + item.src.split(".")[1]}
                    alt={item.alt}
                    style="height: 32px; width: 32px"
                />
            {:else}
                <img src={item.src} alt={item.alt} style="height: 32px; width: 32px" />
            {/if}
        </a>
    {/each}
</section>

<style>
    @media (max-width: 720px) {
        .references {
            display: none;
        }
    }
    .references a {
        display: inline-block;
        margin: 0 0.3rem;
    }
    img:hover {
        filter: none;
        transition: all 0.2s;
        translate: 0 -2px;
    }
</style>
