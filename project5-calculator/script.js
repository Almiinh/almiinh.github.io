...
function handleEqual() {
    if (operator !== "" && storedValue !== "") {
        ...
        document.getElementById("minidisplay").innerText =
            storedValue + " " + operator + " " + display.value + " =";
...
function handleDelete() {
    const display = document.getElementById("display");
    if (["Error", "Infinity", "NaN"].includes(display.value)) handleClearDisplay();
...
function addHover(text) {
    ...
    for (const button of buttons) {
        if (button.innerText.trim() === text.trim()) {
            // Compare innerText after trimming whitespace

            button.classList.add("hover"); // Add the "hover" class that holds your hover styles

            setTimeout(() => {
                button.classList.remove("hover");
            }, 100); // Remove the class after 0.5 seconds (500 milliseconds)
        }
    }
}
