...
const subtitleP = document.querySelector(".warning");
const subtitle2P = document.querySelector(".result");
const player = new Player();
const computer = new Player();
// Utility Functions
function removeAllGridListeners(grid) {
    for (let cell of grid.children) {
        const cellElement = cell;
        const newCursorCell = cellElement.cloneNode(true);
        cellElement.parentNode.replaceChild(newCursorCell, cellElement);
    }
}
/** Get ship cells from cursor cell and call fn function */
function forEachShipCell(length, cursorCell, isVertical, fn) {
...
function getBorderedXY(cell, length, isVertical) {
...
// Event Handlers
function handlePlaceShip(cursorCell, ship, isVertical) {
    try {
        // Place ship
...
function computerTurn() {
    ...
    do {
        x = Math.floor(Math.random() * 10);
        y = Math.floor(Math.random() * 10);
    } while (document.querySelector(`.cell[data-x="${x}"][data-y="${y}"]`).classList.contains("hit") ||
        document.querySelector(`.cell[data-x="${x}"][data-y="${y}"]`).classList.contains("miss"));
    const cell = document.querySelector(`#player-grid .cell[data-x="${x}"][data-y="${y}"]`);
